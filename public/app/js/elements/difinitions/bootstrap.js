	...
	name: 'link',
 	...
 	onEdit: function($scope) {
 		...
 		var left = 0, top = 0,
 			...
 			rightEdge = $scope.inspectorCont.get(0).getBoundingClientRect(),
 			bottomEdge = $('#code-editor-wrapper').get(0).getBoundingClientRect(),
 			leftEdge = $('#elements-container').get(0).getBoundingClientRect(),
 			linkerRight = pos.left + $scope.frameOffset.left + $scope.linker.width(),
 			linkerTop  = pos.top + $scope.frameOffset.top + $scope.linker.height() + 50;
 		...
 		if (bottomEdge.top < linkerTop) {
 			top = pos.top - $scope.selected.node.offsetHeight - $scope.linker.height();
 		} else {
 			top = pos.top + 50 + $scope.selected.node.offsetHeight;
 		}
...
baseBuilderElements.push({
 	...
 	nodes: ['img'],
 	frameworks: ['bootstrap'],
 	html: '<img src="images/builder/profile.png" class="img-responsive">',
...
baseBuilderElements.push({
 	...
 	previewScale: '0.7',
 	previewHtml: '<img data-name="responsive video" data-src="//www.youtube.com/embed/sENM2wA_FTg" class="img-responsive preview-node" src="images/previews/responsiveEmbedPreview.png">',
 	attributes: {
 		url: {
 			text: true,
 			...
 			onChange: function($scope, url) {
 				$($scope.selected.node).data('src', url);
 			}
 		},
 	},
