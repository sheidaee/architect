				...
				if (newProps[prop] && newProps[prop] !== oldProps[prop]) {
					...
					//fill color
						...
						if (newProps[prop] && newProps[prop] != 'transparent' && newProps[prop] !== 'rgba(0, 0, 0, 0)') {
							...
							$scope.previews.color.css('background', 'url("images/transparent.png")');
						}
					}
	...
	return {
		...
		link: function($scope, el, attrs, controller) {
			...
			el.on('click', function(e) {
				var offset = e.currentTarget.offsetTop+$scope.frameOffset.top-19;
				list.removeClass('hidden');
				togglePresetCont(e.currentTarget.id);
				arrow.css('top', offset).show();
				list.css('top', offset-(list.height()/2));
