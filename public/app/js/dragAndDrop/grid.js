...
.factory('grid', ['$rootScope', 'undoManager', function($rootScope, undoManager) {
	...
	var grid = {
		 ...
		 * @param  integer newSpan    column span
		 * 
		 * @return void
		 */
		...
		redrawEditor: function() {
			...
			var editorOffsetTop = 39 + this.active.row.offset().top + this.active.row.height() - $rootScope.frameBody.scrollTop(),
				position = {
			...
			if ($rootScope.frameWrapperHeight - 44 < editorOffsetTop + this.editor.outerHeight()) {
		...
		redrawResizers: function() {
			var top = ((this.editor.get(0).getBoundingClientRect().top - $rootScope.selectBox.get(0).getBoundingClientRect().top) / 2) - 23;
			var r = '<div class="column-resizer"><div class="col-dragger"><i class="fa fa-arrows-h" style="top: '+top+'px"></i></div></div>';
		...
		redrawControls: function() {
      		var middle = '<div class="col-control"><i class="fa fa-trash delete-column"></i><i class="fa fa-plus add-column"></i></div>',
      			first  = '<div class="col-control"><i class="fa fa-plus add-column first"></i><i class="fa fa-trash delete-column first"></i><i class="fa fa-plus add-column"></i></div>',
      			last   = '<div class="col-control"><i class="fa fa-trash delete-column last"></i><i class="fa fa-plus add-column last"></i></div>',
      			oneCol = '<div class="col-control"><i class="fa fa-plus add-column first"></i><i class="fa fa-trash delete-column"></i><i class="fa fa-plus add-column last"></i></div>',
      			control = '';
    ...
    return {
      	...
      	link: function($scope, el) {
      		...
      		grid.editor.find('.close-row-editor').on('click', function(e) {
      			alertify.confirm('این عمل باعث حذف تغییرات اعمال شده خواهد شد، ادامه می دهید؟', function (e) {
		            if (e) {
