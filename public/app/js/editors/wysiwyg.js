	...
	return {
		...
		link: function($scope, el) {
			var textToolbar  = $('#text-toolbar'),
				lastNode     = false,
				toolBarWidth = $('#text-toolbar').outerWidth(),
				toolbarHeight= $('#text-toolbar').outerHeight();

			textToolbar.addClass('hidden');

			var drawWysiwyg = function(x, y, node) {
				var node    = node ? node : $scope.elementFromPoint(x, y - $scope.frameBody.scrollTop()),
  					matched = elements.match(node);
  				...
  				if (matched.canModify.indexOf('text') > -1 && matched.showWysiwyg) {
                    ...
                    var pos   = node.getBoundingClientRect(),
                  	...
                  	if (rightEdge.left < right) {
                  		left = rightEdge.left - $scope.frameOffset.left - toolBarWidth - 25;
                  	}
	...
	return {
		...
		link: function($scope, el) {
			...
			//handle text selection wrapping/unwrapping with link
      		el.find('#link-details > .btn').on('click', function(e) {   			
